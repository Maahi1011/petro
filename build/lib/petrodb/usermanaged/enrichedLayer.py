...
def enrichlayer(modname, libname, configpath, temppath, filepath, filename, deltalake_root, delta_database_name, delta_schema_name, deltaLake_table_name,
                sql_database_name,sql_schema_name,sql_table_name,sql_view_name,dataset_name):
    ...
    # align the enrich delta path according to busniess metadata
    ...
    print("Enriched Delta Path : " + enrich_delta_path)

    # align the backup delta path according to busniess metadata
    backup_delta_path = details_dict.get("DEFAULTS", {}).get(
        "enrich_backup_path") + deltalake_root + '/' + delta_database_name + '/' + delta_schema_name + '/' + deltaLake_table_name + '_' + datetime.today().strftime(
        '%Y%m%d%H%M%S')
    ...
    if check_if_file_exists(modname, libname,enrich_delta_path):
        ...
        if (newdf.count() != 0):
            ...
            # if check_if_file_exists(enrich_backup_path+deltalake_root+'/'+delta_database_name+'/'+delta_schema_name+'/'):
    ...
    dbname=details_dict.get("DEFAULTS", {}).get("dbenricheddbname")
    if dbname in dblist:
        print("db exists")
    else:
    ...
    if tblname in tbllist:
        ...
        spark.sql("CREATE TABLE {}.{} USING DELTA LOCATION '{}'".format(dbname,tblname,enrich_delta_path))
    else:
