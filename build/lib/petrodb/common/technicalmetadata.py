...
def technicalmetadatalayer(modname, libname, configpath, temppath):
    ...
    # df = pd.read_csv(details_dict.get("DEFAULTS", {}).get("tech_metadata_path"))
    ...
    if (filecheck.shape[0] > 0):
        ...
        # truncst = "truncate table " + details_dict.get("DEFAULTS", {}).get("tech_schema_name") + "." + details_dict.get("DEFAULTS", {}).get("tech_table_name") + ";"
        # print(truncst)
        # for index, row in df.iterrows():
        #     insertst = "insert into " + details_dict.get("DEFAULTS", {}).get("tech_schema_name") + "." + details_dict.get("DEFAULTS", {}).get("tech_table_name") + "(filename,columnname,colseq,delta,sql,hashkeycheck,partitionkeycheck,clusterindexcol)values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')".format(
        #         row.filename, row.columnname, row.colseq, row.delta, row.sql, row.hashkeycheck, row.partitionkeycheck,
        #         row.clusterindexcol)
        #     print(insertst)
        #     sqlCursor.execute(insertst)
    else:
        sdf.write.format("com.databricks.spark.sqldw").option("url", sqlDwUrl).option("tempDir", tempDir).option(
            "forwardSparkAzureStorageCredentials", "true").option("dbTable", tbl_name).save()
        # createst = "CREATE TABLE " + details_dict.get("DEFAULTS", {}).get("tech_schema_name") + "." + details_dict.get("DEFAULTS", {}).get("tech_table_name") + "(filename VARCHAR(100),columnname VARCHAR(100),colseq INT,delta VARCHAR(20),sql VARCHAR(20),hashkeycheck CHAR(1),partitionkeycheck CHAR(1),clusterindexcol CHAR(1));"
        # print(createst)
        # sqlCursor.execute(createst)
        # for index, row in df.iterrows():
        #     insertst = "insert into " + details_dict.get("DEFAULTS", {}).get("tech_schema_name") + "." + details_dict.get("DEFAULTS", {}).get("tech_table_name") + "(filename,columnname,colseq,delta,sql,hashkeycheck,partitionkeycheck,clusterindexcol)values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')".format(
        #         row.filename, row.columnname, row.colseq, row.delta, row.sql, row.hashkeycheck, row.partitionkeycheck,
        #         row.clusterindexcol)
